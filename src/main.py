...
def build_binaries(directory):
    ...
    def build_binary(src_fn):
        ...
        if src_fn.endswith('.c'):
            ...
            cmd_line = '%s -g -O2 -std=gnu++17 -static -o %s %s' % (
                gpp, exe_fn, src_fn)
            ...
            skip_py3_check = 'true' if 'solutions' not in src_dir else 'false'
            cmd_line = ('(%s || python3 -m py_compile %s) &&'
                        'ln -sf `pwd`/%s %s'
                        ' && chmod a+x %s' % (skip_py3_check, src_fn, src_fn, exe_fn, exe_fn))
        else:
            ui.show_message(
...
def box_check_output(name):
    ...
    def run_checker(checker_fn):
        u"Run a single checker program against the output file."
